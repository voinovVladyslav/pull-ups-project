...
from .urls import TAG_LIST_URL, get_tags_detail_url
...
def test_only_get_allowed_for_list_view_for_authenticated_user(
    ...
    response = getattr(authenticated_client, method_name)(TAG_LIST_URL)
...
def test_only_get_allowed_for_list_view_for_unauthenticated_user(
    ...
    response = getattr(api_client, method_name)(TAG_LIST_URL)
    assert response.status_code == response_code
...
def test_get_put_patch_delete_allowed_for_unauthenticated_user(
    ...
    response = getattr(api_client, method_name)(get_tags_detail_url(1))
    assert response.status_code == response_code
